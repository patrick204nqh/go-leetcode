...
func TestLinkedListCycle(t *testing.T) {
	testCases := []struct {
		...
		{input: struct {
			values []int
			pos    int
		}{values: []int{3, 2, 0, -4}, pos: 1}, expected: true},
		// No cycle
		{input: struct {
			values []int
			pos    int
		}{values: []int{1, 2}, pos: -1}, expected: false},
		// Single node, no cycle
		{input: struct {
			values []int
			pos    int
		}{values: []int{1}, pos: -1}, expected: false},
		// Empty list, no cycle
		{input: struct {
			values []int
			pos    int
		}{values: []int{}, pos: -1}, expected: false},
		// Cycle at node with value 1
		{input: struct {
			values []int
			pos    int
		}{values: []int{1, 2, 3, 4, 5}, pos: 0}, expected: true},
	}
